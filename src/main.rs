...
mod tests {
    ...
    fn test_worker_threads_cli_flag() {
        ...
        let opts = Opts::try_parse_from(["oha", "http://example.com"]).unwrap();
        assert_eq!(opts.worker_threads.get(), num_cpus::get_physical());

        // A positive value is parsed.
        let opts =
            Opts::try_parse_from(["oha", "--worker-threads", "4", "http://example.com"]).unwrap();
        assert_eq!(opts.worker_threads.get(), 4);

        // Zero and negative values are rejected.
        assert!(
            Opts::try_parse_from(["oha", "--worker-threads", "0", "http://example.com"]).is_err()
        );
        assert!(
            Opts::try_parse_from(["oha", "--worker-threads", "-1", "http://example.com"]).is_err()
        );
    }
}
